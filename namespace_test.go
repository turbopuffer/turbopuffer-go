...
func TestNamespaceBranchFrom(t *testing.T) {
...
func TestNamespaceCopyFromWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestNamespaceExplainQueryWithOptionalParams(t *testing.T) {
	...
	_, err := ns.ExplainQuery(context.TODO(), turbopuffer.NamespaceExplainQueryParams{
		Namespace: turbopuffer.String("namespace"),
		ComputeAttributes: map[string]turbopuffer.Expr{
			"foo": turbopuffer.NewExprHighlight("bar"),
		},
...
func TestNamespaceUpdateMetadataWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
